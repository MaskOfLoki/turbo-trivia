...
export class AnswerCountDown extends ClassBaseComponent {
  ...
  private _gamePoints = 0;
  private _questionTimer = 0;
  private _pointPerQuestionTimerRatio = 1;
  ...
  private stateHandler(value: IState) {
    ...
    this._gamePoints = value.game.gamePoints;
    ...
    this._pointPerQuestionTimerRatio = this._gamePoints / this._questionTimer;
    ...
    if (this._questionIndex !== value.questionIndex) {
      ...
      if (
        ...
        this.timerValue = Math.max(
          Math.floor(
            this._questionTimer + COUNTDOWN_OFFSET_DIFF_FROM_SERVER - (api.time() - value.questionStartTime) / 1000,
          ),
          0,
        );

        this.timerValue = this.timerValue > this._questionTimer ? this._questionTimer : this.timerValue;
        this.pointsValue = Math.round(this.timerValue * this._pointPerQuestionTimerRatio);
